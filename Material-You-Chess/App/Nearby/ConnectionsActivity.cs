...
public abstract class ConnectionsActivity : AppCompatActivity
{
    ...
    /// <summary> Returns the strategy we use to connect to other devices. 
    ...
    /// as well as how much bandwidth is available for use. </summary>
    ...
    protected async void StartDiscovering()
    {
        this.IsDiscovering = true;
        this.DiscoveredEndpoints.Clear();

        var discoveryOptions = new DiscoveryOptions.Builder().SetStrategy(this.Strategy);
        var discovery = this.ConnectionsClient!.StartDiscoveryAsync(this.ServiceId,
            new EndpointDiscoveryCallback(this), discoveryOptions.Build());
    ...
    public virtual void Send(Payload payload) => this.Send(payload, this.EstablishedConnections.Keys);
    ...
    ///<summary> Called when a remote endpoint is discovered.
    /// To connect to the device, call <see cref="ConnectToEndpoint(EndPoint)"/>. </summary>
    ...
    /// <summary> Someone connected to us has sent us data. Override this method to act on the event. </summary>
    ...
    {
        var sent = this.ConnectionsClient!.SendPayloadAsync([.. endpoints], payload);
        await sent;

        if (sent.IsFaulted)
            Logger.Warn($"{nameof(Send)}({nameof(Payload)} {nameof(payload)}) failed. {sent.Exception}");
    }

    private class PayloadCallback(ConnectionsActivity instance) : Android.Gms.Nearby.Connection.PayloadCallback()
    {
        ...
        public override void OnPayloadTransferUpdate(string endpointId, PayloadTransferUpdate update) =>
            Logger.Debug($"{nameof(OnPayloadTransferUpdate)}({nameof(endpointId)}={endpointId}, {nameof(update)}={update}");
    };

    private class EndpointDiscoveryCallback(ConnectionsActivity instance) : Android.Gms.Nearby.Connection.EndpointDiscoveryCallback()
    {
        public override void OnEndpointFound(string endpointId, DiscoveredEndpointInfo info)
        {
            Logger.Debug($"{nameof(OnEndpointFound)}({nameof(endpointId)}={endpointId}, {nameof(info.ServiceId)}={info.ServiceId}," +
                $" {nameof(info.EndpointName)}={info.EndpointName})");
        ...
        public override void OnEndpointLost(string endpointId)
            => Logger.Debug($"{nameof(OnEndpointLost)}({nameof(endpointId)}={endpointId})");
    }

    private class ConnectionLifecycleCallback(ConnectionsActivity instance) : Android.Gms.Nearby.Connection.ConnectionLifecycleCallback()
    {
        public override void OnConnectionInitiated(string endpointId, ConnectionInfo connectionInfo)
        {
            Logger.Debug($"{nameof(OnConnectionInitiated)}({nameof(endpointId)}={endpointId}, " +
