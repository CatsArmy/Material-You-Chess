...
public class PromoteQueen(Pawn origin, ISpace destination) : IPromote
{
    ...
    public NetworkedPromoteQueen ToNetworked() => new(this.Destination.Index, this.Origin.Index, this.OriginPiece.Index);
}

public class NetworkedPromoteQueen((char, int) destination, (char, int) origin, (string, int) originPiece) : INetworkedPromote
{
...
#pragma warning restore CS9124 
    ...
    public IMove FromNetworked(IChessGame game) => new PromoteQueen((game.AllPieces[this.Pawn] as Pawn)!, game.Board[this.Destination]);
}