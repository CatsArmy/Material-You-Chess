...
public class PromoteKnightAndCapture(Pawn origin, IPiece destination) : IPromoteAndCapture
{
    public string Type { get; } = nameof(PromoteKnightAndCapture);
    ...
    public NetworkedPromoteKnightAndCapture ToNetworked() => new(Piece.Index, Destination.Index, Origin.Index, OriginPiece.Index);
}


public class NetworkedPromoteKnightAndCapture((string, int) piece, (char, int) destination, (char, int) origin, (string, int) originPiece)
...
#pragma warning restore CS9124 
    ...
    public IMove FromNetworked(IChessGame game) => new PromoteKnightAndCapture((game.AllPieces[OriginPiece] as Pawn)!, game.AllPieces[Piece]);
}
