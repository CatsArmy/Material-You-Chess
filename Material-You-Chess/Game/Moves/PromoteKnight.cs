...
public class PromoteKnight(Pawn origin, ISpace destination) : IPromote
{
    public string Type { get; } = nameof(PromoteKnight);
    ...
    public NetworkedPromoteKnight ToNetworked() => new(this.Destination.Index, this.Origin.Index, this.OriginPiece.Index);
}


public class NetworkedPromoteKnight((char, int) destination, (char, int) origin, (string, int) originPiece) : INetworkedPromote
{
...
#pragma warning restore CS9124 
    ...
    public IMove FromNetworked(IChessGame game) => new PromoteKnight((game.AllPieces[this.Pawn] as Pawn)!, game.Board[this.Destination]);
}