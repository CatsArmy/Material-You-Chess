...
public class PromoteAndCapture(Pawn origin, IPiece destination) : IPromoteAndCapture
{
    ...
    public NetworkedPromoteAndCapture ToNetworked() => new(Piece.Index, Destination.Index, Origin.Index, OriginPiece.Index);
}


public class NetworkedPromoteAndCapture((string, int) piece, (char, int) destination, (char, int) origin, (string, int) originPiece)
...
#pragma warning restore CS9124 
    ...
    public IMove FromNetworked(IChessGame game) => new PromoteAndCapture((game.AllPieces[OriginPiece] as Pawn)!, game.AllPieces[Piece]);
}
