...
public class PromoteBishop(Pawn origin, ISpace destination) : IPromote
{
    public string Type { get; } = nameof(PromoteBishop);
    ...
    public NetworkedPromoteBishop ToNetworked() => new(this.Destination.Index, this.Origin.Index, this.OriginPiece.Index);
}


public class NetworkedPromoteBishop((char, int) destination, (char, int) origin, (string, int) originPiece) : INetworkedPromote
{
...
#pragma warning restore CS9124 
    ...
    public IMove FromNetworked(IChessGame game) => new PromoteBishop((game.AllPieces[this.Pawn] as Pawn)!, game.Board[this.Destination]);
}
